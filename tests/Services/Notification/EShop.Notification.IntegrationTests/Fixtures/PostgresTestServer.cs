...
/// <summary>
/// Notification audit S1, ported from Payment's fixture of the same name. Owns one PostgreSQL container for the
/// assembly and hands out a fresh database per caller, cloned from a template that the migration chain was applied
/// to once.
///
/// <para>
/// <b>Why Notification needs a real database.</b> On EF InMemory, <c>IdempotentConsumer</c> takes its
/// non-transactional branch, so the relational path production runs — the <c>ON CONFLICT</c> claim and one
/// transaction around <c>HandleAsync</c> that rolls back on any throw — was never exercised here. The InMemory tests
/// asserted a persisted <c>Failed</c> row that Postgres rolls back (audit H1, H2). See
/// <c>Persistence/ConsumerDeliveryRecordTests</c>.
/// </para>
///
/// <para>
/// Two things are load-bearing, as in Payment, Ordering and Catalog. (1) The template: each database is a
/// <c>CREATE DATABASE … TEMPLATE</c> file copy that already carries <c>__EFMigrationsHistory</c>. (2) The pool cap
/// and <see cref="ReleaseDatabase"/>: Npgsql pools per connection string, so without them the server eventually
/// answers <c>53300: sorry, too many clients already</c>.
/// </para>
///
...
{
    private const string TemplateDatabase = "notification_template";
    ...
    public static async Task<string> CreateDatabaseAsync(CancellationToken cancellationToken = default)
    {
        ...
        var databaseName = $"notification_{Guid.NewGuid():N}";
    ...
    private static async Task BuildTemplateAsync(PostgreSqlContainer container, CancellationToken cancellationToken)
    {
        ...
        await using (var context = new NotificationDbContext(
            new DbContextOptionsBuilder<NotificationDbContext>().UseNpgsql(templateConnectionString).Options))
        {
