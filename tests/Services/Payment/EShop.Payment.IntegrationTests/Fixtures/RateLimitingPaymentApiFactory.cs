...
/// <summary>
/// Payment audit Stage 3, ported from Ordering's factory of the same purpose. It turns Payment's rate limiter on
/// under Testing and lowers the limit to something a test can exhaust. It also trusts a simulated gateway, so
/// <c>X-Forwarded-For</c> is honoured, and lets each request choose the client address it presents. Without it the
/// limiter is unreachable from any test, because it is skipped outright under Testing.
/// </summary>
public sealed class RateLimitingPaymentApiFactory : PaymentApiFactory
{
    ...
    /// <summary>
    /// Sets <c>Connection.RemoteIpAddress</c> from <see cref="RemoteIpHeader"/>. Startup filters run ahead of the
    /// application's pipeline, so this lands before UseForwardedHeaders and UseRateLimiter.
    /// </summary>
