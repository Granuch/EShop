...
namespace EShop.Payment.UnitTests.Audit;

/// <summary>
/// Admin panel S15. Every command in Payment's Application assembly is classified — audited or not, with a reason — so a new
...
public class AuditedCommandClassificationTests
{
    ...
    [
        ...
        "ReplayFailedStripeWebhooksCommand",
        "SettleOfflinePaymentCommand",
    ];
    ...
    {
        ["CreatePaymentIntentCommand"] = "a customer starting to pay their own order",
    };

    private static readonly Type[] Requests = typeof(RefundPaymentCommand).Assembly.GetTypes()
